...
import du_an_1_ql_ban_giay.model.HoaDonModel_All;
...
public class HoaDonDAO {
    ...
    public List<HoaDonModel_All> getListHD() {//Lấy hóa đơn fill lên bảng + thanh toán thành công.
        sql = """
         select hd.ID ,kh.hoTen , kh.soDT  , hd.tongGiaTriHoaDon 
       	...
       	LEFT join tbl_khachHang kh on kh.ID = hd.ID_khachHang
              ...
              """;
        List<HoaDonModel_All> listHD = selectBySQL(sql);
        return (!listHD.isEmpty() && listHD != null) ? listHD : null;
    }

    public List<HoaDonModel_All> getListHD_By_HTTT_1And2(int httt) {//fill hóa đơn cho cbb httt
        sql = """
            select hd.ID ,kh.hoTen , kh.soDT  , hd.tongGiaTriHoaDon 
                , tthd.tenTTHD , hd.ngayTao , nv.hoTen  as N'tenNV'
                from tbl_hoaDon hd
                ...
                , hd.tongGiaTriHoaDon  , tthd.tenTTHD , hd.ngayTao , nv.hoTen
              """;
        List<HoaDonModel_All> listHD = selectBySQL(sql, httt);
        return (!listHD.isEmpty() && listHD != null) ? listHD : null;
    }

    public List<HoaDonModel_All> getListHD_By_HTTT_3(int httt) {//fill hóa đơn cho cbb httt
        sql = """
            select hd.ID ,kh.hoTen , kh.soDT  , hd.tongGiaTriHoaDon 
            	, tthd.tenTTHD , hd.ngayTao , nv.hoTen  as N'tenNV'
            	from tbl_hoaDon hd
    ...
    public List<HoaDonModel_All> getListHD_By_NVPT(String nvpt) {
        sql = """
            select hd.ID ,kh.hoTen , kh.soDT  , hd.tongGiaTriHoaDon 
                    , tthd.tenTTHD , hd.ngayTao , nv.hoTen  as N'tenNV'
          	from tbl_hoaDon hd
          	...
          	LEFT join tbl_thanhToan tt on tt.id_hoaDon = hd.ID
          		...
          		, hd.tongGiaTriHoaDon  , tthd.tenTTHD , hd.ngayTao , nv.hoTen
              """;
        List<HoaDonModel_All> listHD = selectBySQL(sql, nvpt);
        return (!listHD.isEmpty() && listHD != null) ? listHD : null;
    }
    ...
    public List<HoaDonModel_All> selectBySQL(String sql, Object... args) {
        List<HoaDonModel_All> listHD = new ArrayList<>();
        ResultSet rs = null;
        try {
...
//          ResultSetMetaData cung cấp thông tin về các cột trong ResultSet.
            ...
            while (rs.next()) {
                HoaDonModel_All hd = new HoaDonModel_All();
            ...
            Logger.getLogger(HoaDonModel_All.class.getName()).log(Level.SEVERE, null, ex);
