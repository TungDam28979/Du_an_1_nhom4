...
public class HĐCT_DAO {
    ...
    public int get_ID_HDCT_BY_ID_HD_And_SPCT(int ID_HĐ, int ID_SPCT) {
        try {
            ...
            Logger.getLogger(HĐCT_DAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public int update(HDCTBanHang e) {//Thêm/giảm SPCT --> update lại SL + giá tiền trả.
    ...
    String update_TTHĐ_KhiXoa = """
                                update tbl_hoaDon
                                set   ID_trangThaiHoaDon = 3
                                where id = ?""";

    public int update_TTHĐ_KhiXoa(int id_HD) {
        return JDBCHelper.update(update_TTHĐ_KhiXoa, id_HD);
    }
